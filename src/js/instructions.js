...
$(document).ready(() => {
  ...
  $.get(
    ...
    function (response) {
      ...
      $("#nutrition").text(`${response.nutrition.properties[3].amount.toFixed(2)}%`);
      $("#calories").text(`${response.nutrition.nutrients[0].amount}kcal`);
      $("#protein").text(`${response.nutrition.caloricBreakdown.percentProtein}%`);
      ...
      $.each(response.extendedIngredients, (index, ingredient) => {
        $("#ingredients").append(`<li class="ingredient fs-5">${ingredient.original}</li>`);
      });
      $.each(response.analyzedInstructions[0].steps, (index, instruction) => {
        $("#instructions").append(`<li class="fs-5 lh-sm">${instruction.step}</li>`);
      });
    }
  );
});