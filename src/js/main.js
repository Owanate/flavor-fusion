...
function getRandomMeal() {
  let meal = "";
  date.getHours() < 12 ? (meal = "breakfast") : date.getHours() < 17 ? (meal = "lunch") : (meal = "dinner");
  getData(meal, (details) => {
    ...
    $("#recipe-link").attr("href", `../pages/instructions.html?id=${details.recipes[0].id}`);
  })
}
...
function getRandomDrink() {
  getData("drink", (data) => {
    ...
    $("#drink-serve").text(`Ready in ${data.recipes[0].readyInMinutes} minutes`);
    $("#drink-image, #drink-modal-image").attr("src", data.recipes[0].image);
    $.each(data.recipes[0].dishTypes, (index, dish) => {
      $("#dish-types-drink").append(`<p class="orange-box rounded px-3 py-2 lh-1 me-sm-3">${dish}</p>`);
    });
    $.each(data.recipes[0].extendedIngredients, (index, ingredient) => {
      $("#drink-ingredients").append(`<li class="ingredient fs-5">${ingredient.original}</li>`);
    });
    $.each(data.recipes[0].analyzedInstructions[0].steps, (index, instruction) => {
      $("#drink-instructions").append(`<li class="fs-5 lh-1 mb-2">${instruction.step}</li>`);
    });
  });
}
...
function getRandomSnack() {
  getData("snack", (data) => {
    ...
    $("#snack-serve").text(`Ready in ${data.recipes[0].readyInMinutes} minutes`);
    $("#snack-image, #snack-modal-image").attr("src", data.recipes[0].image);
    $.each(data.recipes[0].dishTypes, (index, dish) => {
      $("#dish-types-snack").append(`<p class="orange-box rounded px-3 py-2 lh-1 ms-sm-3">${dish}</p>`);
    });
    $.each(data.recipes[0].extendedIngredients, (index, ingredient) => {
      $("#snack-ingredients").append(`<li class="ingredient fs-5">${ingredient.original}</li>`);
    });
    $.each(data.recipes[0].analyzedInstructions[0].steps, (index, instruction) => {
      $("#snack-instructions").append(`<li class="fs-5 lh-1 mb-2">${instruction.step}</li>`);
    });
  });
}
